...
def parse_interactive() -> tuple[
  ...
  print(
    "Paste editor export, then press Enter twice (or Ctrl+D):",
    file=sys.stderr,
  )
  lines: list[str] = []
  blank_count = 0
  ...
  wall_flags: list[int] | None = None
  entities: dict[str, tuple[int, int]] | None = None
  red: bool | None = None
  exit_info: dict[str, str | int] | None = None
...
def main() -> None:
  ...
  parser.add_argument(
    "--exit",
    type=str,
    help='exit info as JSON, e.g. {"side":"N","pos":3}',
  )
  parser.add_argument("--red", action="store_true", help="level uses red mummies")
  parser.add_argument(
    "--top", type=int, default=5, help="number of top matches to show"
  )
  parser.add_argument(
    "--interactive",
    "-i",
    action="store_true",
    help="paste editor output",
  )
  ...
  if args.interactive:
    ...
    entities = (
      {k: tuple(v) for k, v in json.loads(args.entities).items()}
      if args.entities
      else {}
    )
    red = args.red
    exit_info = json.loads(args.exit) if args.exit else None  # noqa: F841
    grid_size = None
  ...
  if grid_size is None:
    print(
      f"Cannot infer grid size from {n_cells} cells.",
      file=sys.stderr,
    )
    sys.exit(1)
  ...
  # Collect matches
  best: list[tuple[int, int, int, int, int, str, bool]] = []
  dat_files = sorted(
    args.maze_dir.glob("B-*.dat"),
  ...
  print(
    f"Target: {N}x{N}, {'red' if red else 'white'} mummies, {len(entities)} entities"
  )
  print()
  ...
  for wall_match, ent_score, ent_total, fi, si, tlabel, flip in best:
    ...
    print(
      f"B-{fi} sub-{si}: {wall_match}/{N * N} walls ({pct}%),"
      f" entities={ent_score}/{ent_total},"
      f" transform={tlabel}, type={flip_str}"
    )

    parsed = parse_file(args.maze_dir / f"B-{fi}.dat")
    assert parsed is not None
    level = parsed.sublevels[si]
