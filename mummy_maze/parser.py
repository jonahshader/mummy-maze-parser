...
def parse_sublevel(data: bytes, offset: int, header: dict):
    """Parse one sub-level from the data stream at the given offset.

    Returns a dict with:
      cells: list[list[int]] - wall flags per cell, cells[row][col]
      exit_side: str - "N", "S", "E", or "W"
      exit_pos: int - position along the exit side
      entities: dict[str, tuple[int, int]] - name -> (col, row)
      flip: bool - True if red mummies (vertical-first)
    """
    gs = header["grid_size"]
    N = gs
    pos = offset
    ...
    if not flip:
        ...
        for r in range(N):
            for c in range(N):
                v = cells[r][c]
                tv = 0
                if v & WALL_NORTH: tv |= WALL_WEST
                if v & WALL_SOUTH: tv |= WALL_EAST
                if v & WALL_WEST:  tv |= WALL_NORTH
                if v & WALL_EAST:  tv |= WALL_SOUTH
                transposed[c][r] = tv
        cells = transposed
    else:
        ...
        for r in range(N):
            for c in range(N):
                v = cells[r][c]
                fv = 0
                if v & WALL_NORTH: fv |= WALL_NORTH
                if v & WALL_SOUTH: fv |= WALL_SOUTH
                if v & WALL_EAST:  fv |= WALL_WEST
                if v & WALL_WEST:  fv |= WALL_EAST
                flipped[r][N - 1 - c] = fv
        cells = flipped
    ...
    for t in range(header["scorpion"]):
        entities[f"scorpion{t + 1}" if header["scorpion"] > 1 else "scorpion"] = read_entity()

    for t in range(header["trap_count"]):
        entities[f"trap{t + 1}" if header["trap_count"] > 1 else "trap"] = read_entity()
...
def render_maze(level, grid_size):
...
def parse_file(filepath: Path):
    """Parse an entire .dat file, returning (header, sublevels) or None."""
    ...
    sublevels = []
    offset = 6
    for i in range(header["num_sublevels"]):
